...
try:
    ...
    try:
        ...
        sys.exit("[!] 检测到安装错误（缺少模块）. 访问 'https://github.com/sqlmapproject/sqlmap/#installation' 浏览更多详情")
    ...
    warnings.filterwarnings(action="ignore", message="不再支持Python 2")
    warnings.filterwarnings(action="ignore", message=".*已经导入", category=UserWarning)
    warnings.filterwarnings(action="ignore", message=".*使用非常旧的版本", category=UserWarning)
...
def checkEnvironment():
    try:
        ...
        errMsg = "您的系统不能正确处理非ascii路径. "
        errMsg += "请将sqlmap的目录移动到另一个位置"
        logger.critical(errMsg)
        raise SystemExit

    if LooseVersion(VERSION) < LooseVersion("1.0"):
        errMsg = "您的运行时环境 (e.g. PYTHONPATH) 为 "
        errMsg += "坏了。请确保您没有运行"
        errMsg += "新版本的sqlmap和运行时脚本"
        errMsg += "版本"
        logger.critical(errMsg)
        raise SystemExit
...
def main():
    ...
    try:
                        ...
                        if conf.crawlDepth and conf.bulkFile:
                            ...
                            for i in xrange(len(targets)):
                                ...
                                try:
                                    ...
                                    infoMsg = "启动目标URL的爬虫 '%s' (%d/%d)" % (target, i + 1, len(targets))
                                    logger.info(infoMsg)

                                    crawl(target)
                                except Exception as ex:
                                    if target and not isinstance(ex, SqlmapUserQuitException):
                                        errMsg = "爬虫时出现问题 '%s' ('%s')" % (target, getSafeExString(ex))
                                        logger.error(errMsg)
                                    else:
                                        raise
                                else:
        ...
        if any(_ in excMsg for _ in ("MemoryError", "Cannot allocate memory")):
            ...
            errMsg = "无效URL (违反 Bidi IDNA 规则 - RFC 5893)"
            logger.critical(errMsg)
            raise SystemExit
            ...
            errMsg = "在Python 3.11的ALPHA版本中运行sqlmap时存在一个已知的问题。"
            errMsg += "请降级到稳定的Python版本"
            logger.critical(errMsg)
            raise SystemExit

        elif all(_ in excMsg for _ in ("Resource temporarily unavailable", "os.fork()", "dictionaryAttack")):
            errMsg = "运行多进程哈希破解时出现了一个问题。 "
            errMsg += "请重新运行选项 '--threads=1'"
            logger.critical(errMsg)
            raise SystemExit

        elif "can't start new thread" in excMsg:
            errMsg = "创建新线程实例时出现了一个问题。 "
            errMsg += "请确保你没有太多的进程在运行"
            if not IS_WIN:
            ...
            errMsg = "在常规的套接字操作中出现了一个问题 "
            errMsg += "('%s')" % excMsg.strip().split('\n')[-1]
            logger.critical(errMsg)
            raise SystemExit

        elif all(_ in excMsg for _ in ("pymysql", "configparser")):
            errMsg = "检测到 'pymsql' 初始化错误(使用Python3依赖项)"
            logger.critical(errMsg)
            raise SystemExit

        elif all(_ in excMsg for _ in ("ntlm", "socket.error, err", "SyntaxError")):
            errMsg = "检测到 'python-ntlm' 初始化错误 (使用Python2语法)"
            logger.critical(errMsg)
            raise SystemExit

        elif all(_ in excMsg for _ in ("drda", "to_bytes")):
            errMsg = "检测到'drda' 初始化错误 (使用Python3语法)"
            logger.critical(errMsg)
            raise SystemExit
            ...
            errMsg = "本地会话文件似乎有问题. 请使用 '--flush-session'重新运行"
            logger.critical(errMsg)
            raise SystemExit

        elif "AttributeError: 'module' object has no attribute 'F_GETFD'" in excMsg:
            errMsg = "无效运行 (\"%s\") " % excMsg.split("Error: ")[-1].strip()
